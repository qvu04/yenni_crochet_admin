...
import type { CustomRequestStatusFilter } from '../../../services'
import { customRequestStatusOptions } from '../../../utils'

interface CustomRequestFiltersProps {
  searchInput: string
  status: CustomRequestStatusFilter
  onSearchInputChange: (value: string) => void
  onSubmitSearch: () => void
  onStatusChange: (status: CustomRequestStatusFilter) => void
}

export const CustomRequestFilters = ({
  searchInput,
  status,
  onSearchInputChange,
  onSubmitSearch,
  onStatusChange,
}: CustomRequestFiltersProps) => {
    ...
    <div className="flex w-full flex-col gap-3 xl:w-auto xl:min-w-[700px] xl:flex-row xl:items-center xl:justify-end">
      <form className="relative flex-1" onSubmit={handleSubmit}>
        ...
        <input
          ...
          placeholder="Tìm khách, SĐT hoặc mô tả"
      ...
      <SegmentedControl value={status} options={customRequestStatusOptions} onValueChange={onStatusChange} />
    </div>
  )
}
